...
import { CABINETS, ALL_DRUGS, PHARMACY_INFO } from './catalog.js?v=ph0664';
import { DRUG_PLACEMENT } from './drug-placement.js?v=ph0664';
import { createCharacter } from './character.js?v=ph0665';
...
export function buildScene(canvas, opts = {}) {
  ...
  basket.traverse(o => { if (o.isMesh) { o.castShadow = true; o.userData = { retailBasketClick: true }; } });
  basket.userData = { retailBasketClick: true };
  scene.add(basket);
  propLabel('GIỎ RA LẺ', 0.24, '#0ea5e9', -0.78, propY + 0.11, COUNTER_Z - 0.13);
  ...
  function applyHdsdSticker(drugId, label, placement) {
    ...
    if (isBottle) {
      ...
      const sw = Math.min(faceW * 0.62, faceH * 0.8 * (HDSD_W / HDSD_H));
