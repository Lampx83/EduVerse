...
// ============================================================
// EduVerse plugs in sibling apps (ScoreUp, Codelab, Smartdoc, FeedBackMe)
...
// DENY (e.g. Codelab via Helmet) can still render inside EduVerse's iframe.
//
 ...
 * @property {string} [backHref]  link back into EduVerse on the down-page
 */
  ...
  app.use(mount, (req, res) => {
    ...
    const proxyReq = http.request(
      ...
      (proxyRes) => {
        ...
        // Allow embedding inside EduVerse's same-origin iframe.
...
/** Drop the `frame-ancestors …;` directive so the app can be framed by EduVerse. */
...
<body style="font-family:system-ui,sans-serif;background:#0b1220;color:#e5edf7;padding:48px;line-height:1.7;max-width:680px;margin:auto">
  ...
  <p>EduVerse đang reverse-proxy <strong>${label}</strong> tại <code>${target}</code> nhưng tiến trình không phản hồi.</p>
  ${startHint ? `<p>Khởi động: <code>${startHint}</code></p>` : ''}
  <p style="opacity:.55;font-size:13px">Chi tiết: ${String((err && err.message) || err)}</p>
  <p><a style="color:#38bdf8" href="${backHref}">← Quay lại EduVerse</a></p>
</body></html>`;
}
